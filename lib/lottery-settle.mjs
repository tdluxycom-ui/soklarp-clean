...
import { VN_FAST_REVEAL_MS, VN_REVEAL_HOLD_MS } from "./vn-reveal.mjs";
import { padLast, parseXienNumbers, XIEN_SIZES } from "./bet-numbers.mjs";
...
export function createLotteryEngine({
  ...
  async function resolveVNDraw(lotteryId, prizes) {
    ...
    for (const bet of currentBets) {
      let isWin = false;
      const num = String(bet.numbers ?? "");
      if (XIEN_SIZES[bet.betType]) {
        const parsed = parseXienNumbers(bet.betType, num);
        if (!parsed.ok) {
          // A slip that does not carry the required distinct numbers cannot be
          // priced at the xiên rate. Void it and hand the stake back rather
          // than paying out a slip an earlier build should have rejected.
          bet.status = "cancelled";
          bet.payout = 0;
          const owner = db.users.find((u) => u.username === bet.username);
          if (owner) owner.balance = (owner.balance || 0) + Number(bet.amount || 0);
          console.warn(`Voided malformed ${bet.betType} slip ${bet.id} ("${num}"): ${parsed.reason}`);
          continue;
        }
        isWin = parsed.numbers.every((p) => lo2Set.has(p));
      } else {
        switch (bet.betType) {
          case "lo":
  ...
  async function settleLotteryRoom(key) {
    ...
    try {
      ...
      if (lottery.type === "vietlottery") {
        if (lottery.liveDraw) {
          const started = Number(lottery.liveDraw.startedAt) || Date.now();
          const duration = Number(lottery.liveDraw.durationMs) || VN_FAST_REVEAL_MS;
          if (Date.now() < started + duration + VN_REVEAL_HOLD_MS) {
            lottery.countdown = 0;
            return;
          }
        ...
        if (key === "vnfast" && lottery.interval) {
          lottery.liveDraw = {
            drawId: lottery.nextDrawId,
            prizes: generateVNPrizes(),
            startedAt: Date.now(),
            durationMs: VN_FAST_REVEAL_MS
          };
          lottery.countdown = 0;
          console.log(`VN FAST REVEAL [${lottery.nextDrawId}] G1 → ĐB in 90s`);
          await saveDb();
          return;
        }
